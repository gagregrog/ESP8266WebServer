...
void setup(void) {
  ...
  // Filesystem status
  server.on("/status", __HTTP_GET, handleStatus);

  // List directory
  server.on("/list", __HTTP_GET, handleFileList);

  // Load editor
  server.on("/edit", __HTTP_GET, handleGetEdit);

  // Create file
  server.on("/edit",  __HTTP_PUT, handleFileCreate);

  // Delete file
  server.on("/edit",  __HTTP_DELETE, handleFileDelete);

  // Upload file
  // - first callback is called after the request has ended with all parsed arguments
  // - second callback handles file upload at that location
  server.on("/edit",  __HTTP_POST, replyOK, handleFileUpload);
